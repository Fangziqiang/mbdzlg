...
(function($) {
					...
					if (data.rows != null) {
						$.each(data.rows, function(i, row) {
							...
							$(ths).each(function(j) {
								...
								// ***********
									if (idx == "-2") { // checkbox
										div.push("<input type='checkbox' id='chk_", row.id, "' name='' class='itemchk' value='", row.id, "'/>");
										if (tdclass != "") {
											tdclass += " chboxtd";
										} else {
											tdclass += "chboxtd";
										...
										if (idx == "-1") { // rownum
											var row_num_line = (p.page - 1) * p.rp + i + 1;
											div.push(p.rownumText + row_num_line);
										} else {
					...
					$("rows row", data).each(function() {
						...
						$(ths).each(function(j) {
							...
							if (idx == "-1") { // checkbox
									...
									if (this.process) {
										divInner = this.process(divInner, trid, arrdata);
									}
									div.push(divInner);
								}
								div.push("</div>");
	...
	changeSort : function(th) { // change sortorder
		...
		if (p.colModel) {
			...
			// ****************
			if (p.rownum) {
				var rth = jQuery('<th>' + p.rownumDisplay + '</th>');
				rth.addClass("rth").attr( {
					'axis' : "col-1",
					width : p.rownumWidth,
					align : 'center',
					"isch" : true
		...
		if (p.usepager) {
			...
			// title="转到第一页"><span></span></div><div class="pPrev pButton"
			// title="转到上一页"><span></span></div> </div><div
			// class="btnseparator"></div> <div class="pGroup"><span
			// class="pcontrol">当前第 <input type="text" size="1" value="1"
			// />页，总页数 <span> 1 </span></span></div><div
			// class="btnseparator"></div><div class="pGroup"> <div class="pNext
			// pButton" title="转到下一页"><span></span></div><div class="pLast
			// pButton" title="转到最后一页"><span></span></div></div><div
			// class="btnseparator"></div><div class="pGroup"> <div
			// class="pReload pButton" title="刷新"><span></span></div> </div>
			// <div class="btnseparator"></div><div class="pGroup"><span
			// class="pPageStat"></span></div>';
			var html = '<div class="pGroup"><div class="pFirst pButton" title="转到第一页"><span></span></div><div class="pPrev pButton" title="转到上一页"><span></span></div> </div><div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol"><input type="text" size="1" value="1" />页，总 <span> 1 </span></span></div><div class="btnseparator"></div><div class="pGroup"> <div class="pNext pButton" title="转到下一页"><span></span></div><div class="pLast pButton" title="转到最后一页"><span></span></div></div><div class="btnseparator"></div><div class="pGroup"> <div class="pReload pButton" title="刷新" style="display:none"><span></span></div> </div> <div class="btnseparator"></div><div class="pGroup"><span class="pPageStat"></span></div>';
			...
			if (p.searchitems) {
				...
				$(g.sDiv).append(
						"<div class='sDiv2'>快速检索：<input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>" + sopt
		...
		if ($('th', g.hDiv).length) {
			...
			$('th div', g.hDiv).each(
					function() {
						...
						$('tbody', g.nDiv).append(
								'<tr><td class="ndcol1"><input type="checkbox" ' + chk + ' class="togCol noborder" value="' + cn + '" /></td><td class="ndcol2">' + this.innerHTML + '</td></tr>');
						cn++;
					});
	...
	$.fn.flexReload2 = function(p) { // function to reload grid 创建人：刘军平
		return this.each(function() {
			if (p.url != "") {
				this.p.url = p.url;
			}
	...
	// Mignet's code 通过指定行数和列数取得数据
	$.fn.getCellByCoordinate = function(rowid, colid) { // function to get row's
		// data from grid
		var griddata = {
			total : 1,
			page : 1,
