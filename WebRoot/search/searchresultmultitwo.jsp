...
<body onload="onload();" scroll="no">
	...
	function inarray(obj, arr) {
	 ...
	 function onload(){
			...
			//$("#tableId").find("tr:not(eq(1))").attr("class","");
			//$("#tableId tr:not(eq(0))").find("td:eq(2)").attr("class","");
			var selected = document.getElementById("selected").value.split(",");
			
			$("#tableId input").each(function(){
				if($(this).val()!=""&&inarray($(this).val(), selected)){
					$(this).parent().parent().children('td:first').children('input[name="mxitems"]').attr('checked','checked');
				}
			});
		}
	//全选标识
	...
	function add(){
		...
		if(item){
			...
			if($("input[type=checkbox][name=mxitems][checked]").length==0){
				parent.newSearchLine("<%=request.getParameter("forward") %>");		
				var fieldArray = document.getElementsByName("searchfields")[0].value;
				var targetArray = document.getElementsByName("targetfields")[0].value;
				var targetobj = document.getElementsByName("consistfields")[0].value;
				...
				if (fieldArray) {
					...
					for (var i = 0; i < targets.length; i++) {
						if(targets[i] == fields[i]){
							multiString[i] = "";
							parent.putValue(targets[i],multiString[i]);
						}
					}
				}	
			}
	...
	<s:form  method="post" onsubmit="checkparamter();">
		...
		<s:hidden name="selected" id="selected" value="%{#parameters.selected[0]}" />
	...
	<input type="hidden" name="curItem"/>
	 <div class="search">
               ...
               <div class="overY" style="overflow-x: auto;">
                 <table width="100%" id="tableId" border="0" cellspacing="0" cellpadding="0" class="tab">
					...
					<tr class="" align="center">
						...
						<s:iterator id="title" status="idxid" value="#request.titles">
							<td class="list_table_title_middle" nowrap><s:property value="title" /></td>
						</s:iterator>
					</tr>
					<s:iterator id="result" value="#request.list">
						<tr class="list_table_rows_tr list_table_rows_td" title="请选中后确定"style="cursor:hand" align="center">
							<td class="list_table_content">
