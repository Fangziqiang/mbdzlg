...
var TabPane = this.TabPane = new Class({
	...
	// tab is exist or not 
	isExist: function(isTab){		 
		var li = $('ulid').getElements('li'); 	
		var flag = false;		
		for(var i =0;i<li.length;i++){ 
			if(isTab.id == li[i].id){	
				//alert("tabselector="+this.options.tabSelector);
				var tab = this.container.getElements(this.options.tabSelector)[i];
				//alert("tab = "+tab);
				var content = this.container.getElements(this.options.contentSelector)[i];
				flag = true;				
				return this.activeTab(tab,content,i);
			}
		};
		if(!flag){
			return false;
	...
	//active tab
	activeTab: function(tab,content,i){	
		if (tab) {
