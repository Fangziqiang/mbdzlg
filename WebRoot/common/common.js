...
/*选择行政区划  resultColumn为回传字段名称，例如'xzqhmc,xzqhdm'中间用逗号隔开，valueColumn为获得的类型'mc,dm',xzqh为查询条件*/
function getXzqhByShowModalDialog(resultColumn, valueColumn, xzqh) {
	var url = '../xzqh/selectXzqh.action?resultColumn=' + resultColumn + '&valueColumn=' + valueColumn + '&condition=' + xzqh;

	window.showModalDialog(url,window, 'dialogWidth=580px;dialogHeight=270px;scroll=no');		
	 //window.open(url, "", "width= 200px, height=300px, top=0, left=0, toolbar=no, menubar=no, resizable=yes, location=no, status=no");
}
